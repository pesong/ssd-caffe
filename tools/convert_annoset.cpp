...
//   ....
// For segmentation task, the file should be in the format as
//   imgfolder1/img1.JPEG annofolder1/anno1.xml
//   ....
...
DEFINE_string(anno_type, "classification",
    "The type of annotation {classification, detection, segmentation}.");
...
int main(int argc, char** argv) {
  ...
  // pesong  add file,lablefile pairs
  std::vector<std::pair<std::string, std::string > > lines_label;
  std::string filename;
  ...
  if (anno_type == "classification") {
    ...
    CHECK(MapNameToLabel(label_map, check_label, &name_to_label))
        << "Failed to convert name to label.";
    while (infile >> filename >> labelname >> labelimagename) {
      lines.push_back(std::make_pair(filename, labelname));
      lines_label.push_back(std::make_pair(filename, labelimagename));
    }
  }
  ...
  for (int line_id = 0; line_id < lines.size(); ++line_id) {
    ...
    if (anno_type == "classification") {
      ...
      labelimagename = root_folder + lines_label[line_id].second;
      labelname = root_folder + boost::get<std::string>(lines[line_id].second);

      //pesong 将image 和 两个label转为datum   caffe/src/util/io.cpp
      status = ReadRichImageToAnnotatedDatumWithSeg(filename, labelimagename, labelname, resize_height,
            resize_width, min_dim, max_dim, is_color, enc, type, label_type,
            name_to_label, &anno_datum);
